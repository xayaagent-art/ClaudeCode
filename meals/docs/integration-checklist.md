...
| Reasoning | Both tasks run at `low`, overridable with `OPENAI_RECEIPT_REASONING` / `OPENAI_MEAL_REASONING` (`none`, `low`, `medium`, `high`, `xhigh`). Transcription wants `minimal` in principle, and production said: `400 Unsupported value: 'minimal' is not supported with the 'gpt-5.6-luna' model`. At `low` a prompt with nothing to think about billed zero reasoning tokens, so `low` costs nothing and is accepted; `none` is the floor if wanted. A model that rejects the parameter entirely is detected from its 400 and retried without it, once per process |
